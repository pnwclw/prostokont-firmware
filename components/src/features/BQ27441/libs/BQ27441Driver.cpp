...
BQ27441Driver::BQ27441Driver()
    : _deviceAddress(BQ72441_I2C_ADDRESS), _sealFlag(false),
      _userConfigControl(false), _i2c_handle(NULL) {}
...
bool BQ27441Driver::begin(i2c_master_dev_handle_t dev_handle) {
  ...
  uint16_t deviceID = deviceType(); // Read deviceType from BQ27441
...
uint16_t BQ27441Driver::flags(void) { return readWord(BQ27441_COMMAND_FLAGS); }
...
bool BQ27441Driver::writeExtendedData(uint8_t classID, uint8_t offset,
...
int16_t BQ27441Driver::i2cReadBytes(uint8_t subAddress, uint8_t *dest,
                                    uint8_t count) {
  esp_err_t ret = i2c_master_transmit_receive(_i2c_handle, &subAddress, 1, dest,
                                              count, BQ72441_I2C_TIMEOUT);
  return (ret == ESP_OK) ? BQ72441_I2C_TIMEOUT : 0;
}

// Write a specified number of bytes over I2C to a given subAddress
uint16_t BQ27441Driver::i2cWriteBytes(uint8_t subAddress, uint8_t *src,
                                      uint8_t count) {
  // Pack register address + data into a single buffer (max 33 bytes: 1 addr +
  // 32 data)
  uint8_t buf[33];
  buf[0] = subAddress;
  memcpy(buf + 1, src, count);
  esp_err_t ret =
      i2c_master_transmit(_i2c_handle, buf, count + 1, BQ72441_I2C_TIMEOUT);
  return (ret == ESP_OK) ? 1 : 0;
}

// BQ27441Driver lipo; // Use lipo.[] to interact with the library in an Arduino
// sketch
