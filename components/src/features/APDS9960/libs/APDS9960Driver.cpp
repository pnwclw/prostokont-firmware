...
int APDS9960Driver::readGesture() {
  ...
  while (1) {
    ...
    vTaskDelay(pdMS_TO_TICKS(FIFO_PAUSE_TIME));
    ...
    if ((gstatus & APDS9960_GVALID) == APDS9960_GVALID) {
      ...
      if (fifo_level > 0) {
        bytes_read = wireReadDataBlock(APDS9960_GFIFO_U, (uint8_t *)fifo_data,
                                       (fifo_level * 4));
        if (bytes_read == -1) {
          return ERROR;
        }
 ...
 * @return True if successful write operation. False otherwise.
 */
bool APDS9960Driver::wireWriteDataBlock(uint8_t reg, uint8_t *val,
                                        unsigned int len) {
  uint8_t *buf = new uint8_t[len + 1];
  buf[0] = reg;
  memcpy(buf + 1, val, len);
  esp_err_t ret = i2c_master_transmit(dev_handle_, buf, len + 1, 100);
  delete[] buf;
  return ret == ESP_OK;
}
...
bool APDS9960Driver::wireReadDataByte(uint8_t reg, uint8_t &val) {
  return i2c_master_transmit_receive(dev_handle_, &reg, 1, &val, 1, 100) ==
         ESP_OK;
}
 ...
 */
int APDS9960Driver::wireReadDataBlock(uint8_t reg, uint8_t *val,
                                      unsigned int len) {
  if (i2c_master_transmit_receive(dev_handle_, &reg, 1, val, len, 100) !=
      ESP_OK) {
    return -1;
  }
  return (int)len;
